...
<div class="header" style="display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:12px;margin-bottom:30px;">
    <h1 style="font-size:24px;font-weight:700;">Promoções</h1>
    <a href="<?= url_to('admin_promotions_create') ?>" class="btn"
        style="background:#f43f5e;color:white;padding:10px 20px;border-radius:12px;font-weight:600;font-size:14px;box-shadow: 0 4px 12px rgba(244, 63, 94, 0.2);">
        + Lançar Promoção
    </a>
</div>
...
<div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:16px;margin-bottom:30px;">
    <div class="card" style="padding:18px;text-align:center;">
        <p style="font-size:11px;color:#64748b;text-transform:uppercase;margin-bottom:6px;">Total Promoções</p>
        <p style="font-size:22px;font-weight:700;color:white;"><?= number_format((int)($summary['total_lots'] ?? 0)) ?></p>
    </div>
    <div class="card" style="padding:18px;text-align:center;">
        <p style="font-size:11px;color:#64748b;text-transform:uppercase;margin-bottom:6px;">Quantidade Total</p>
    ...
    <div class="card" style="padding:18px;text-align:center;">
        <p style="font-size:11px;color:#64748b;text-transform:uppercase;margin-bottom:6px;">Lucro Estimado</p>
    ...
    <div style="background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:18px 20px;">
        <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;">
            ...
            <div style="display:flex;gap:8px;align-items:flex-end;padding-bottom:1px;">
                <button type="submit"
                    style="background:#f43f5e;color:white;border:none;border-radius:8px;padding:8px 18px;font-size:13px;font-weight:600;cursor:pointer;white-space:nowrap;">
                    Filtrar
                </button>
                <a href="<?= current_url() ?>"
        ...
        <table style="width:100%;border-collapse:collapse;">
            <thead>
                <tr style="border-bottom:1px solid rgba(255,255,255,0.06);">
                    ...
                    <th style="padding:14px 20px;text-align:right;font-size:11px;color:#64748b;text-transform:uppercase;font-weight:600;">Quantidade</th>
                    ...
                    <th style="padding:14px 20px;text-align:right;font-size:11px;color:#64748b;text-transform:uppercase;font-weight:600;">Taxa Custo</th>
                    <th style="padding:14px 20px;text-align:right;font-size:11px;color:#64748b;text-transform:uppercase;font-weight:600;">Taxa Promoção</th>
            ...
            <tbody>
                <?php if (empty($lots)): ?>
                    <tr>
                        <td colspan="11" style="padding:40px;text-align:center;color:#64748b;font-size:14px;">Nenhuma promoção encontrada com os filtros aplicados.</td>
                    </tr>
                <?php else: ?>
                        ...
                        <tr style="border-bottom:1px solid rgba(255,255,255,0.04);transition:background 0.15s;"
                            ...
                            <td style="padding:14px 20px;">
                                <?php if ($lot['delivery_type']): ?>
                                    <span style="font-size:11px;padding:3px 8px;border-radius:20px;background:rgba(244,63,94,0.12);color:#f87171;font-weight:700;"><?= strtoupper($lot['delivery_type']) ?></span>
                                <?php else: ?>
                            ...
                            <td style="padding:14px 20px;text-align:right;font-size:13px;color:#f87171;font-weight:700;">R$ <?= number_format((float)$lot['promo_rate'], 4, ',', '.') ?></td>
                            ...
                            <td style="padding:14px 20px;text-align:right;">
                                <div style="display:flex;align-items:center;gap:12px;justify-content:flex-end;">
                                    <a href="<?= url_to('admin_lots_show', $lot['id']) ?>"
                                        style="font-size:12px;color:#f43f5e;text-decoration:none;font-weight:600;white-space:nowrap;">Ver →</a>
                                    
                                    <form action="<?= url_to('admin_promotions_deactivate', $lot['id']) ?>" method="POST" onsubmit="return confirm('Deseja realmente desativar esta promoção? Ela sumirá do dashboard de todos os clientes.');" style="display:inline;margin:0;">
                                        <?= csrf_field() ?>
                                        <button type="submit"
                                            style="background:none;border:none;color:#ef4444;font-size:12px;font-weight:600;cursor:pointer;padding:0;white-space:nowrap;">
                                            Desativar
                                        </button>
                                    </form>
                                </div>
...
<style>
    ...
    .page-btn.page-active { background: #f43f5e; color: white; border-color: #f43f5e; font-weight: 600; }
